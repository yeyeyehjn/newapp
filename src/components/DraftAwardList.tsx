...
export default function DraftAwardList({ onBack, onSelectItem }: DraftAwardListProps) {
  ...
  const [statusFilter, setStatusFilter] = useState<'pending' | 'drafted'>('pending');
  ...
  const statusTabs: { label: string; value: 'pending' | 'drafted' }[] = [
    { label: '待核阅', value: 'pending' },
    { label: '已核阅', value: 'drafted' },
  ];
  ...
  return (
    <div className="absolute inset-0 bg-slate-50 z-50 flex flex-col animate-slide-in text-left">
      ...
      <div className="h-12 bg-[#ddecff] border-b border-slate-100 flex items-center px-4 relative flex-shrink-0">
        ...
        <div className="absolute left-1/2 -translate-x-1/2 text-base font-bold text-slate-800 whitespace-nowrap">草拟裁决书</div>
      </div>
