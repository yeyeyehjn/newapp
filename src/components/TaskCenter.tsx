    ...
    return (
      <div className="flex-1 bg-slate-50 flex flex-col min-h-0 overflow-y-auto w-full text-left">
        ...
        <div className="bg-white border-b border-slate-100 p-3.5 flex items-center justify-between shadow-xs sticky top-0 z-10">
          ...
          <span className="text-[11px] bg-slate-100 text-slate-700 font-black px-2.5 py-1 rounded-xl font-mono tracking-wider">
                  ...
                  <div key={task.id} className="bg-white rounded-3xl border border-slate-100 shadow-sm p-4 relative overflow-hidden flex flex-col space-y-3">
                    <div className="flex justify-between items-start">
                      <div className="space-y-0.5">
                        <span className="text-[9px] font-mono font-bold text-slate-400 block">{task.caseNo}</span>
                        <h4 className="text-xs font-black text-slate-705">{task.title}</h4>
                      </div>
                      <span className={`text-[8.5px] font-black px-2 py-0.5 rounded-lg border ${
                    ...
                    {isTaskPending ? (
                      <button
                        ...
                        className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-heavy text-[10px] py-2.5 rounded-xl transition-all shadow-xs flex items-center justify-center gap-1"
                      >
                        <i className="fa-solid fa-calendar-check text-[9px]"></i>
                        <span>立即进入勾选排期 ➔</span>
                      </button>
                    ) : (
                      <div className="bg-slate-50 rounded-xl p-2.5 text-center text-[10px] text-slate-400 font-bold border border-slate-100 flex items-center justify-center gap-1">
                        <i className="fa-solid fa-circle-check text-emerald-500"></i>
                        <span>此排期任务已于刚才递交完成</span>
                      </div>
                    )}

                    <div className="flex justify-between items-center text-[9px] text-slate-400 font-medium pt-2 border-t border-dashed border-slate-100">
              ...
              {tasks.filter(t => t.type === 'sign').map(task => {
                ...
                return (
                  <div key={task.id} className="bg-white rounded-3xl border border-slate-100 shadow-sm p-4 relative overflow-hidden flex flex-col space-y-3">
                    ...
                    <p className="text-[10.5px] text-slate-455 leading-relaxed text-justify">
                      {task.description}
                    </p>
                    ...
                    {isTaskPending ? (
                      <button
                        ...
                        className="w-full bg-red-500 hover:bg-red-600 text-white font-heavy text-[10px] py-2.5 rounded-xl transition-all shadow-md shadow-red-500/10 flex items-center justify-center gap-1"
                      >
                ...
                <div key={tr.id} className="bg-white rounded-3xl border border-slate-100 shadow-sm p-4 space-y-3.5 relative overflow-hidden text-left shadow-inner">
                  <div className="flex justify-between items-start">
                    <div className="space-y-0.5 max-w-[80%]">
                      <span className="text-[8.5px] text-slate-450 block font-mono">创建时间: {tr.date}</span>
                      <h4 className="text-[11px] font-black text-slate-800 leading-snug">{tr.title}</h4>
                    </div>
                  ...
                  {!tr.signed ? (
                    <button
                      ...
                      className="w-full bg-purple-600 hover:bg-purple-700 text-white font-heavy text-[10px] py-2 rounded-xl transition-all font-sans flex items-center justify-center gap-1 shadow-sm"
                    >
                    ...
                    <div className="bg-emerald-500/10 border border-emerald-300/30 rounded-xl p-2.5 flex items-center gap-2">
                      <div className="w-5 h-5 rounded-full bg-emerald-500 text-slate-900 flex items-center justify-center text-[10px] font-black">
                        ✓
                      </div>
                      <div className="text-[9.5px] text-emerald-800">
                        <strong>已于系统通过! </strong> 
                        防伪核注哈希：<span className="font-mono text-[8px] bg-white border px-1">0xBF0812_{tr.id}</span>
                      </div>
                    </div>
                  )}
                </div>
            ...
            <div className="space-y-4 animate-fade-in">
              ...
              {extensions.map(ext => (
                <div key={ext.id} className="bg-white rounded-3xl border border-slate-100 shadow-sm p-4 space-y-3 relative overflow-hidden">
                  <div className="flex justify-between items-start">
                    <div className="space-y-0.5">
                      <span className="text-[8.5px] font-mono text-amber-700 font-bold bg-amber-50 px-1 py-0.2 rounded border border-amber-100">{ext.caseNo}</span>
                  ...
                  <div className="bg-slate-50 rounded-xl p-3 text-[10px] text-slate-500 leading-normal space-y-1.5 border border-slate-100">
                    <p><strong>申请代理人:</strong> <span className="text-slate-700 font-bold">{ext.applicant}</span></p>
                    <p className="text-justify text-[9.5px]"><strong>延期原因:</strong> {ext.reason}</p>
                    <p className="text-[8px] font-mono text-slate-400">递送时间: {ext.applyDate}</p>
                  </div>
                  ...
                  {ext.status === 'pending' && (
                    <div className="flex gap-2">
                      <button
                        ...
                        className="flex-1 bg-slate-100 hover:bg-slate-200 text-slate-500 font-bold py-2 text-[10px] rounded-xl transition-colors cursor-pointer text-center"
                      >
                        驳回申请
                      </button>
                      <button
                        onClick={() => handleExtensionDecision(ext.id, 'approved')}
                        className="flex-1 bg-[#1E40AF] hover:bg-[#1E40AF]/95 text-white font-heavy py-2 text-[10px] rounded-xl transition-colors cursor-pointer text-center shadow-sm"
                      >
                        审批通过并按期扣除
                      </button>
                    </div>
            ...
            <div className="space-y-4 animate-fade-in">
              ...
              {promises.map(p => (
                <div key={p.id} className="bg-white rounded-3xl border border-slate-100 shadow-sm p-4 space-y-3 relative overflow-hidden">
                  ...
                  <p className="text-[10px] text-slate-500 leading-normal text-justify bg-slate-50/60 p-2.5 rounded-xl border border-slate-100">
                    {p.desc}
                  </p>
                  ...
                  {!p.signed ? (
                    <button
                      ...
                      className="w-full bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white font-heavy text-[10px] py-1.8 py-2 rounded-xl transition-all flex items-center justify-center gap-1 shadow-sm"
                    >
                      <i className="fa-solid fa-fingerprint text-[11px]"></i>
                      <span>CA授权一键核章 ➔</span>
                    </button>
                  ) : (
                    <div className="bg-emerald-500/10 border border-emerald-400/20 rounded-xl p-2.5 flex items-center gap-1.5 text-[10px] text-emerald-800 font-bold">
        ...
        {signingTranscriptId && (
          <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-[110] flex items-center justify-center p-4">
            <div className="bg-white border rounded-3xl max-w-sm w-full p-5 shadow-2xl relative space-y-4 animate-scale-up text-slate-800 text-left">
              ...
              <div className="space-y-0.5">
                <span className="text-[8.5px] bg-[#1E40AF]/10 text-[#1E40AF] border border-[#1E40AF]/30 px-2 py-0.5 rounded font-black font-mono">
                ...
                <div className="bg-slate-50 border p-2 rounded-lg text-[9px] font-mono text-slate-600 truncate mt-1">
                ...
                <div className="space-y-3">
                  {!isPinVerified ? (
                    <div className="space-y-2">
                      <label className="text-[10px] font-bold text-slate-500 block">请输入CA盾保护交易Key</label>
                      <div className="flex gap-2">
                        <input
                          type="password"
                          value={pinCode}
                      ...
                      {pinError && <p className="text-[9.5px] text-red-500 font-bold">{pinError}</p>}
                    </div>
                  ) : (
                    <div className="space-y-3">
                      <div className="flex items-center justify-between">
                        <span className="text-[10px] font-bold text-slate-500">双因子印签：张明</span>
                        <div className="flex gap-1">
                          <button
                            onClick={() => setSignatureMode('auto')}
                            className={`p-1 px-2 text-[9px] font-extrabold rounded ${signatureMode === 'auto' ? 'bg-indigo-50 text-indigo-600 border border-indigo-200' : 'bg-slate-100 text-slate-500'}`}
                          >
                            系统预存
                          </button>
                          <button
                            onClick={() => setSignatureMode('draw')}
                            className={`p-1 px-2 text-[9px] font-extrabold rounded ${signatureMode === 'draw' ? 'bg-indigo-50 text-indigo-600 border border-indigo-200' : 'bg-slate-100 text-slate-500'}`}
                          >
                            手绘签字
                          </button>
                        </div>
                      ...
                      {signatureMode === 'draw' ? (
                        <div className="space-y-2 bg-slate-50 border rounded-lg p-2">
                          ...
                          />
                          <button onClick={clearCanvas} className="text-[8.5px] text-slate-400 underline block text-right">清空手写</button>
                        </div>
                      ) : (
                  ...
                  <button 
                    ...
                    className="w-full text-center text-[10px] text-slate-400 py-1"
                  >
                    取消并返回
                  </button>
                </div>
    ...
    return (
      <div className="flex-1 bg-slate-50 flex flex-col min-h-0 overflow-y-auto w-full text-slate-800 text-left">
        ...
        <div className="bg-white border-b border-slate-100 p-3.5 flex items-center justify-between shadow-xs sticky top-0 z-10">
          ...
          <span className="text-[11px] bg-red-100 text-red-800 font-black px-2.5 py-1 rounded-xl font-mono tracking-wider">
        ...
        <div className="p-4 space-y-4">
          <div className="bg-white rounded-3xl border border-slate-100 shadow-xl p-5 space-y-4 relative overflow-hidden">
            ...
            {/* Header info */}
            <div className="space-y-0.5">
              <span className="text-[9px] bg-indigo-50 text-indigo-600 border border-indigo-100/60 px-1 py-0.2 rounded font-black font-mono">
                TASK WORKFLOW STAGE
              </span>
            ...
            {selectedTask.type === 'sign' && !isStampAdded && (
              <div className="bg-amber-50 rounded-xl p-3 border border-amber-100 flex items-start gap-2 text-[10.0px] text-amber-700 leading-relaxed">
            ...
            {selectedTask.type === 'sign' && (
              <div className="space-y-4">
                <div className="border border-slate-200 rounded-lg p-3 bg-slate-50/50 max-h-[240px] overflow-y-auto text-[10.5px] pr-2 text-justify relative line-clamp-10 leading-relaxed">
                  {isStampAdded && (
                    <div className="absolute right-6 bottom-4 pointer-events-none select-none animate-scale-up z-20">
                      <div className="w-20 h-20 rounded-full border-[3px] border-red-600 bg-red-50/10 flex flex-col items-center justify-center text-center relative select-none">
                        <div className="text-[6.5px] text-red-600 font-extrabold tracking-[0.2em] transform -rotate-12 select-none">
                          广州仲裁委员会
                        </div>
                        <div className="text-[9.5px] text-red-600 font-bold m-0.5">★</div>
                        <div className="text-[8px] text-red-600 font-extrabold tracking-widest select-none">
                          裁决专章
                        </div>
                        <div className="absolute inset-0 flex items-center justify-center font-serif text-[22px] font-bold text-red-600/70 select-none transform rotate-12">
                          张明
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="whitespace-pre-line font-serif pl-1.5 text-[10px]">
                ...
                {isStampAdded ? (
                  <div className="pt-2 flex flex-col items-center justify-center space-y-2 text-center animate-fade-in bg-slate-50 p-4 rounded-lg border border-slate-100">
                    ...
                    <h5 className="text-[11px] font-bold text-emerald-600">正在生成司法区块链电子存证及签名戳记...</h5>
                  ...
                  <div className="space-y-3 pt-1">
                    {!isPinVerified ? (
                      <div className="bg-slate-50 p-3 rounded-xl border border-slate-100 space-y-2">
                        <label className="text-[10px] font-bold text-slate-500 block">请输入证书CA盾授权保护密码</label>
                        <div className="flex gap-2">
                          <input
                            type="password"
                            value={pinCode}
                      ...
                      <div className="space-y-3">
                        <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-1.5">
                          <span className="text-[10px] font-extrabold text-slate-600">✓ 已授权：首席特聘仲裁员张明盾签库</span>
                          <div className="flex gap-1">
                            <button
                              onClick={() => setSignatureMode('auto')}
                              className={`p-1 px-2 text-[10px] font-extrabold rounded ${signatureMode === 'auto' ? 'bg-indigo-50 text-indigo-600 border border-indigo-200' : 'bg-slate-100 text-slate-500'}`}
                            >
                              系统预存印章
                            </button>
                            <button
                              onClick={() => setSignatureMode('draw')}
                              className={`p-1 px-2 text-[10px] font-extrabold rounded ${signatureMode === 'draw' ? 'bg-indigo-50 text-indigo-600 border border-[#1E40AF]/20' : 'bg-slate-100 text-slate-500'}`}
                            >
                              手绘盖印格式
                            </button>
                          </div>
                        ...
                        {signatureMode === 'draw' ? (
                          <div className="space-y-2 bg-slate-50 border rounded-lg p-2">
                            <p className="text-[9px] text-slate-400 italic">在下框内书写您的中文公章规范姓名：</p>
                            <canvas
                              ref={canvasRef}
                              width={320}
                              height={100}
                            ...
                            <div className="flex justify-end">
                              <button onClick={clearCanvas} className="text-[9px] text-slate-400 underline">清空内容</button>
                            </div>
                          </div>
                        ) : (
            ...
            {selectedTask.type === 'schedule' && (
              <div className="space-y-4">
                ...
                <div className="bg-slate-50 p-3 rounded-xl border text-[9.5px] text-slate-400 font-medium">
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2 bg-slate-50 p-3 rounded-xl border">
                  {selectedTask.details?.reviewDocs?.map((doc) => (
                    ...
                    >
                      <span className="text-[10.5px] font-bold text-slate-700 flex items-center space-x-1.5">
                ...
                <div className="space-y-1.5 text-left">
                  <label className="text-[10px] font-bold text-slate-500 block">仲裁辅助合议实质意见备忘签注（选填）</label>
                  <textarea
                    rows={3}
                    ...
                    className="w-full bg-slate-50 text-[10.5px] p-2 rounded-xl border outline-none focus:border-indigo-505"
                  />
                </div>
    ...
    <div className="flex-1 bg-slate-100 flex flex-col overflow-hidden relative">
      ...
      <div className="flex-1 overflow-y-auto px-4 pt-6  pb-8 space-y-4 no-scrollbar">
        ...
        <div 
          ...
          className="bg-white rounded-xl p-5 shadow-sm border border-slate-50 transition-all duration-150 active:scale-[0.98] cursor-pointer hover:shadow-md hover:border-red-100 group text-left"
        >
          <div className="flex justify-between items-center mb-4">
            <div className="flex items-center space-x-3">
              ...
              <h2 className="font-bold text-slate-800 text-[14px]">待开庭提醒</h2>
            </div>
          ...
          <div className="grid grid-cols-3 gap-2 py-3 border-t border-b border-slate-50 mb-3 text-center">
            <div>
              <div className="text-sm font-bold text-red-600">{hearingsStats.today}</div>
              <div className="text-[10px] text-slate-400 tracking-wider">今日</div>
            </div>
            <div className="border-x border-slate-50">
              <div className="text-sm font-bold text-amber-500">{hearingsStats.near}</div>
              <div className="text-[10px] text-slate-400 tracking-wider">临近</div>
            </div>
            <div>
              <div className="text-sm font-bold text-green-600">{hearingsStats.scheduled}</div>
              <div className="text-[10px] text-slate-400 tracking-wider">已排期</div>
            </div>
          </div>
          ...
          <div className="space-y-1 text-right">
            ...
            <div className="flex justify-between select-none">
              ...
              <span className="text-[10px] text-slate-400 font-medium">本周开庭完成率 {hearingsStats.near === 0 ? '100%' : '62%'}</span>
            </div>
          </div>
        </div>
        ...
        <div 
          ...
          className="bg-white rounded-xl p-5 shadow-sm border border-slate-50 transition-all duration-150 active:scale-[0.98] cursor-pointer hover:shadow-md hover:border-blue-100 group text-left"
        >
          <div className="flex justify-between items-center mb-4">
            <div className="flex items-center space-x-3">
              ...
              <h2 className="font-bold text-slate-800 text-[14px]">结案文书签名</h2>
            </div>
            <div className="flex items-center gap-2">
          ...
          <div className="grid grid-cols-2 gap-2 py-3 border-t border-b border-slate-50 mb-3 text-center">
            <div className="border-r border-slate-50">
              <div className="text-sm font-bold text-blue-600">{signaturesStats.pending}</div>
              <div className="text-[10px] text-slate-400 tracking-wider">待签</div>
            </div>
            <div>
              <div className="text-sm font-bold text-sky-600">{signaturesStats.signed}</div>
              <div className="text-[10px] text-slate-400 tracking-wider">本月已签</div>
            </div>
          </div>
          <div className="w-full bg-slate-100 h-1.5 rounded-full overflow-hidden">
              <div className="bg-blue-600 h-full transition-all duration-500" style={{ width: hearingsStats.near === 0 ? '100%' : '62%' }}></div>
            </div>

          <div className="flex justify-between mt-3 text-[10px] text-slate-400 font-sans tracking-wide">
            <span>本年度应签 6 份，已签 {6 - pendingPromisesSignaturesCount} 份</span>
            <span className="text-[10px] text-slate-400 font-medium">完成率 {Math.round(((6 - pendingPromisesSignaturesCount) / 6) * 100)}%</span>
          </div>
        </div>
        ...
        <div 
          ...
          className="bg-white rounded-xl p-5 shadow-sm border border-slate-50 transition-all duration-150 active:scale-[0.98] cursor-pointer hover:shadow-md hover:border-purple-100 group text-left"
        >
          <div className="flex justify-between items-center mb-4">
            <div className="flex items-center space-x-3">
              ...
              <h2 className="font-bold text-slate-800 text-[14px]">笔录签名</h2>
            </div>
            <div className="flex items-center gap-2">
          ...
          <div className="grid grid-cols-2 gap-2 py-3 border-t border-slate-50 mb-1 text-center">
            <div className="border-r border-slate-50">
              <div className="text-sm font-bold text-purple-600">{pendingTranscriptsSignaturesCount}</div>
              <div className="text-[10px] text-slate-400 tracking-wider">待签署</div>
            </div>
            <div>
              <div className="text-sm font-bold text-slate-400">{transcripts.filter(t => t.signed).length}</div>
              <div className="text-[10px] text-slate-400 tracking-wider">已签署</div>
            </div>
          </div>
        </div>
        ...
        <div 
          ...
          className="bg-white rounded-xl p-5 shadow-sm border border-slate-50 transition-all duration-150 active:scale-[0.98] cursor-pointer hover:shadow-md hover:border-amber-100 group text-left"
        >
          <div className="flex justify-between items-center mb-4">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h2 className="font-bold text-slate-800 text-[14px]">延期办理审批</h2>
              </div>
            </div>
            <div className="flex items-center gap-2">
          ...
          <div className="grid grid-cols-2 gap-2 py-3 border-t border-slate-50 mb-1 text-center font-sans">
            <div>
              <div className="text-sm font-bold text-amber-600">{pendingExtensionsRequestsCount}</div>
              <div className="text-[10px] text-slate-400 tracking-wider">待审批</div>
            </div>
            <div className="border-l border-slate-50">
              <div className="text-sm font-bold text-green-600">{extensions.filter(e => e.status !== 'pending').length}</div>
              <div className="text-[10px] text-slate-400 tracking-wider">已审批</div>
            </div>
          </div>
        </div>
        ...
        <div 
          ...
          className="bg-white rounded-xl p-5 shadow-sm border border-slate-50 transition-all duration-150 active:scale-[0.98] cursor-pointer hover:shadow-md hover:border-emerald-105 group text-left"
        >
          <div className="flex justify-between items-center mb-3">
            <div className="flex items-center space-x-3">
              ...
              <h2 className="font-bold text-slate-800 text-[14px]">承诺书签署</h2>
            </div>
            <div className="flex items-center gap-2">
